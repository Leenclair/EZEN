...
class Person implements Comparable<Person>{
	...
	public int compareTo(Person p) {
		//정렬기준을 나이로 정하자
		if(this.age>p.age) {
			return 1;//매개변수 p의 나이가 작다면 양수
		}else if(this.age<p.age) {
			return -1;//매개변수 p의 나이가 많다면 음수
		}else {
			return 0;//같으면 0
		}
	}
...
public class ComparableTest1 {
	public static void main(String[] args) {
		...
		//TreeSet<T> 는 Person인스턴스가 저장될 때마다 기존에 저장된 인스턴스와의 비교를 위해서
		//comparableTo() 메서드를 빈번히 호출하여, 이때 반환되는 값을 기반으로 정렬을 진행
